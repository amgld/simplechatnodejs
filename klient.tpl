...
// Запрашиваем разрешение на вибрацию и уведомления
navigator.vibrate([100, 50, 100]);
if (Notification) if (Notification.permission != "granted")
   Notification.requestPermission();

// Функция оправляет уведомление в браузер клиента
function notify(title, text) {
   ...
   if (Notification.permission == "granted") new Notification(title, opt);
      ...
      Notification.requestPermission(function(p) {
         if (p == "granted") new Notification(title, opt);
      });
}
...
function putBeseda() {
   ...
   var zaprs = new XMLHttpRequest(); zaprs.abort();
   zaprs.open('POST', '?'+pwd+'^', true);
   zaprs.onreadystatechange = function() {
      if (zaprs.readyState == 4) if(zaprs.status == 200) zaprs.abort();
   };
...
function getBeseda() {
   var zapros = new XMLHttpRequest(); zapros.abort();
   zapros.open('GET', '?'+pwd+'^', true);
   zapros.onreadystatechange = function() {
      if (zapros.readyState == 4) if(zapros.status == 200) {
         beseda = zapros.responseText;
         var txtBes = document.getElementById('bes');
         txtBes.innerHTML = beseda;
         if (beseda.length != besedaLength) {
            besedaLength = beseda.length;
            var newNotif = beseda.split('</p>')[0].split('<br>')[1];         
            if (document.hidden) notify("Новое сообщение чата", newNotif);
         }
      }
   };
   zapros.send(null);
